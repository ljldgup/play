...
from kof.attention_related import Attention, SelfAttention, PositionalEncoding, BahdanauAttention


# 输入配合raw_env_data_to_input使用
def general_input(model):
    ...
    last_action_embedding = layers.Embedding(model.action_num, 16, name='last_action_embedding')(last_action)
    ...
    return model_input, encoder_input, query_input


# 多层rnn堆叠
def build_stacked_rnn_model(model):
    model_input, encoder_input, query_input = general_input(model)
...
def build_rnn_attention_model(model):
    # 基于rnn的编码器
    model.network_type = 'rnn_coder_model'
    model_input, encoder_input, query_input = general_input(model)
...
def build_multi_attention_model(model):
    ...
    important_layers = [role1_actions_embedding, role2_actions_embedding,
                        role_position, conv_role_position, role_distance, conv_role_distance
                        ]
    unimportant_layers = [role_abs_distance, role1_energy_embedding, role2_energy_embedding,
